...
namespace ProductManagement.Addresses
{
    ...
    public class AddressesAppService : ApplicationService, IAddressesAppService
    {
        ...
        {
            var totalCount = await _addressRepository.GetCountAsync(input.FilterText, input.CIty, input.State, input.PostalCodeMin, input.PostalCodeMax, input.Country, input.UserId);
            var items = await _addressRepository.GetListAsync(input.FilterText, input.CIty, input.State, input.PostalCodeMin, input.PostalCodeMax, input.Country, input.UserId, input.Sorting, input.MaxResultCount, input.SkipCount);
        ...
        public virtual async Task<AddressDto> CreateAsync(AddressCreateDto input)
        {
            ...
            input.CIty, input.State, input.PostalCode, input.Country, input.UserId
            );
        ...
        public virtual async Task<AddressDto> UpdateAsync(Guid id, AddressUpdateDto input)
        {
            ...
            id,
            input.CIty, input.State, input.PostalCode, input.Country, input.UserId, input.ConcurrencyStamp
            );
